...
.. todo: need new examples below: one for "Request with Headers: JSON" and one for "Request with Headers: YAML".*

.. In the request example below, notice that *``Content-Type``* is set to *``application/json``*, but *``application/yaml``* is requested via the *``Accept``* header:

.. Therefore a YAML response format is returned:
